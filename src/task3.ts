...
/**
 * Функция нормализует входящие данные
 * @param {Array} unnormalizedData - массив постов с обязательным свойством id в каждом
 * @returns 
 */
const normalizeData = (unnormalizedData: PostInterface[]) => {
  const byId: Record<string, PostInterface> = {}
    unnormalizedData.forEach(post => {
        byId[post.id] = post
    });
  const allIds = unnormalizedData.map((post: PostInterface)=> {
    return post.id
  })
  return ({byId,allIds})
};
